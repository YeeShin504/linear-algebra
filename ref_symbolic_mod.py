...
class Matrix(sym.MutableDenseMatrix):
    ...
    def create_vander(num_rows: int = 1, 
                      num_cols: int = 1, 
                      symbol: str = 'x', 
                      is_real: bool = True) -> sym.Matrix:
    ...
    def apply_vander(self, x: sym.Matrix) -> sym.Matrix:
    ...
    def identify(self,
                 tolerance: float = None) -> sym.Matrix:
    ...
    def insert_aug_line(self, pos: int = -1) -> sym.Matrix:
        if pos == -1:
            pos = self.cols
        self.augpos.add(pos)
        return self
    ...
    def evaluate_cases(self, rhs: sym.Matrix) -> None:
        ...
        for i, case in enumerate(cases, 1):
            ...
            U = self.row_join(rhs).subs(case).ref(verbosity = 0, matrices = 1, follow_GE = False)
            display(U)
    ...
    def sep_part_gen(self) -> tuple[sym.Matrix, sym.Matrix]:
    ...
    def inverse(self, 
        ...
        if option is not None:
            X = Matrix.create_unk_matrix(num_rows = self.cols, 
                                         num_cols = self.rows, 
                                         symbol = 'x')
            if option == 'left':
    ...
    def QRdecomposition(self, 
                        full: bool = False,
                        *args, **kwargs) -> tuple[sym.Matrix, sym.Matrix]:
    ...
    def orthogonally_diagonalize(self,
                                 reals_only: bool = True,
                                 factor: bool = True,
                                 verbosity = 1,
                                 *args, **kwargs) -> tuple[sym.Matrix, sym.Matrix]:
        ...
        P, D = self.diagonalize(reals_only=reals_only, 
                                verbosity=verbosity, 
                                *args, **kwargs)
        d = defaultdict(list)
        ...
        for val, vecs in d.items():
            if len(vecs) > 1:
                ...
                if factor:
                    ...
                    result.append(Matrix.from_list(vecs).gram_schmidt(factor, verbosity))
            else:
    ...
    def singular_value_decomposition(self, 
                                     verbosity: int = 0,
                                     *args, **kwargs) -> tuple[sym.Matrix, sym.Matrix, sym.Matrix]:
        ...
        if verbosity >= 1:
            ...
            for idx, vec, val in zip(range(1, S.rows + 1), V.columnspace(), sigma):
                if val != 0:
                    u_i = self @ vec / val
                    u_list.append(u_i)
                    display(IPython.display.Math(f"u_{idx} = (1/{sym.latex(val)})A{sym.latex(vec)} = {sym.latex(u_i)}"))
    ...
    def standard_matrix(self, 
                        out: sym.Matrix,
                        matrices: int = 1) -> sym.Matrix:
        # returns the standard matrix for the transformation from self to out
        X = Matrix.create_unk_matrix(num_rows = out.rows, 
                                     num_cols = self.rows)
