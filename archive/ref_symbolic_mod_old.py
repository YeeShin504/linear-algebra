...
class Matrix(sym.MutableDenseMatrix):
    ...
    def rref(self) -> tuple[sym.Matrix, tuple[int]]:
        my_rref, pivots = super().rref()
    ...
    def column_constraints(self, 
        ...
        unknowns = sym.symbols(f"x:{self.rows+1}")
        vector = Matrix(list(unknowns)[1:])

        # insert hidden column vectors so that the unknown vector is not reduced in rref
        if use_id:
            hidden = self.id()
        elif len(self.orthogonal_complement()) == 0:
            hidden = self.id()
        else:
    ...
    def nullspace(self, 
        ...
        if self.rank() == self.cols:
            ...
            # I can't figure out why I cannot just call super().nullspace()
            res = sym.Matrix(self).nullspace()
            return res
