...
def sympy_commands():
    ...
    >>> A.diagonalize(reals_only: bool = True) # returns tuple of invertible P and diagonal D
...
class Matrix(sym.MutableDenseMatrix):
    ...
    def QRdecomposition(self, 
        ...
        if full and Q.rows != Q.cols:
            Q = Matrix(Q)
            Q_aug = Q.row_join(Q.id()).QR_decomposition()[0]
    ...
    def orthogonally_diagonalize(self,
        ...
        for val in d:
            ...
            if factor:
                ortho, norm = d[val].gram_schmidt(factor, verbosity)
                result.append(ortho @ norm)
            else:
                result.append(d[val].gram_schmidt(factor, verbosity))
        if len(result) == 0:
            return (P, D)
        else:
            ortho_P = result[0]
