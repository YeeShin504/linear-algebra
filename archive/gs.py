...
def sympy_commands():
    ...
    >>> A.simplify(rational: bool = True, simplify: bool = True, expand: bool = True, collect_sym: sym.Symbol = None)
...
class Matrix(sym.MutableDenseMatrix):
    ...
    def adjoint(self, *args, **kwargs) -> sym.Matrix:
        # point adjoint to classical adjoint (which is defined as adjugate in sympy)
    ...
    def gram_schmidt(self) -> List[sym.Matrix]:
        if self.cols == 0:
            return []
        if is_IPython():
    ...
    def simplify(self,
        ...
        temp = self.copy()
        if rational:
            temp = sym.nsimplify(temp, tolerance = 0.001, rational = True)
        if simplify:
